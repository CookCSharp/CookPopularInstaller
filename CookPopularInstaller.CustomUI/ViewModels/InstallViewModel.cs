    ...
    public class InstallViewModel : ViewModelBase
    {
        ...
        private async Task UninstallPreviousProduct()
        {
            await Task.Run(() =>
            {
                ...
                if (isExistProduct)
                {
                    string path = RegistryHelper.GetLocalMachineRegistryKeyPathValue($"SOFTWARE\\{companyName}\\{productName}", "Directory")?.ToString();
                    Toolkit.Helpers.ProcessHelper.StartProcess(Path.Combine(path, "Uninst.exe"), "/quiet /uninstall");
                }
            });
        }
