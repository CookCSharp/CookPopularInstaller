...
def write_extensions(root:Element):
    ...
    with open(f'CommandLine/{package_json}','r',encoding='utf-8') as f:
        content = json.load(f)
        content = str(content).replace('[InstallFolder]', '[INSTALLFOLDER]').replace('[CompanyName]', '!(loc.CompanyName)').replace('[ProductName]', '!(loc.ProductName)').replace('[AppPath]', '[#App.exe]').replace('[ProductVersion]', '$(var.AppVersion)')
        content = eval(content)      
        environment_variables = content['Extensions']['EnvironmentVariables']
        registry_variables = content['Extensions']['RegistryVariables']
...
def update_product_variable_value():
    ...
    versionElement = ProcessingInstruction(f'define AppVersion={package_version}')
...
def update_directory(app_file_name:str):
    ...
    with open(f'CommandLine/{package_json}','r',encoding='utf-8') as f:
        content = json.load(f)
        services = content['Extensions']['WindowsServices']
