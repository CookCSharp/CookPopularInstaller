...
namespace NCATestInstaller.Admin
{
    internal class Program
    {
        private const int SizeToRawDataAddress = 0x000002B8;
        ...
        static void ElevateAdministratorByModifyPE(string path)
        {
            try
            {
                //var path = @"D:\Users\chance.zheng\Desktop\Company\NCATestInstaller\Output\bin\Debug\x86\NCATestInstaller.CustomUI.Exe\NCATestInstaller.Generate-Setup.exe";
                ...
                //小端模式
                var sizeToRawDataValue = BitConverter.ToInt32(buffer, SizeToRawDataAddress);
        ...
        {
            var name = "NCATestInstaller_" + Process.GetCurrentProcess().GetHashCode();
            ...
            var packageName = "NCATestInstaller.exe";
            ...
            string Extract(string name)
            {
                ...
                using var stream = assembly.GetManifestResourceStream($"NCATestInstaller.Admin.{name}");
        ...
        {
            if (args.Length == 1)
                ElevateAdministratorByModifyPE(args[0]);

            //ElevateAdministratorByWrapper();
        }
    }
}