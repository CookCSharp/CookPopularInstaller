...
namespace NCATestInstaller.Toolkit
{
    public class Build
    {
        ...
        public static void Configure(string packageJsonFileName)
        {
            ...
            ConfigurationManageHelper.ModifyItem(Environment.CurrentDirectory + "\\NCATestInstaller.CustomUI.exe", "PackageTheme", project.PackageTheme.ToString(), "appSettings");
            ConfigurationManageHelper.ModifyItem(Environment.CurrentDirectory + "\\NCATestInstaller.CustomUI.exe", "LicenseFileName", project.PackageLicenseName, "appSettings");
            ConfigurationManageHelper.ModifyItem(Environment.CurrentDirectory + "\\NCATestInstaller.CustomUI.exe", "ContainChineseOnInstallFolder", project.ContainChineseOnInstallFolder, "appSettings");
            ConfigurationManageHelper.ModifyItem(Environment.CurrentDirectory + "\\NCATestInstaller.CustomUI.exe", "Language", project.Language.ToString(), "appSettings");
            //string uninstallProjectPath = Path.Combine(Environment.CurrentDirectory, "..\\..\\NCATestInstaller.Uninstall");
        ...
        public static IList<string> GetBuildArguments(PackageInfo package, string packageJsonFileName)
        {
            ...
            var wixprojFilePath_Msi = Environment.CurrentDirectory + "\\Assets\\Msi\\NCATestInstaller.Msi.wixproj";
            ...
            var csharpCustomActionTargetFileName = "NCATestInstaller.CSharpCustomAction";
            ...
            var argument4 = $"\"{candleExePath}\" \"{wxsFilePath_Msi}\" {arch} -dNCATestInstaller.CSharpCustomAction.TargetDir=\"{csharpCustomActionTargetDir}\" -dNCATestInstaller.CSharpCustomAction.TargetName={csharpCustomActionTargetFileName} -out \"{wixprojFilePath_Msi}\"";
            ...
            if (package.Project.PackageType.ToString().Contains("Exe"))
            {
                ...
                var wixprojFilePath_Exe = Environment.CurrentDirectory + @$"\Assets\{package.Project.PackageType}\NCATestInstaller.{package.Project.PackageType}.wixproj";
                var cultureFilePath_Exe = Environment.CurrentDirectory + @$"\Assets\{package.Project.PackageType}\zh-cn.wxl";
                var adminFilePath = Environment.CurrentDirectory + @"\Admin\NCATestInstaller.Admin.exe";

                string argument6 = string.Empty;
                if (package.Project.PackageType == PackageType.Exe)
                {
                    argument6 = $"\"{candleExePath}\" \"{wxsFilePath_Exe}\" {arch} -ext \"{wixBalExtension}\" -dNCATestInstaller.Msi.TargetPath=\"{outputFileName_Msi}\" -out \"{wixprojFilePath_Exe}\"";
                }
                else
                {
                    var customUITargetDir = Environment.CurrentDirectory + @"\\"; //此处结尾需要加双斜杠，否则candle.exe时会报错
                    var customUITargetFileName = "NCATestInstaller.CustomUI.exe";
                    var customUITargetPath = Environment.CurrentDirectory + @$"\{customUITargetFileName}";
                    argument6 = $"\"{candleExePath}\" \"{wxsFilePath_Exe}\" {arch} -ext \"{wixBalExtension}\" -ext \"{wixNetFxExtension}\" -ext \"{wixUtilExtension}\" -dNCATestInstaller.CustomUI.TargetDir=\"{customUITargetDir}\" -dNCATestInstaller.CustomUI.TargetFileName=\"{customUITargetFileName}\" -dNCATestInstaller.CustomUI.TargetPath=\"{customUITargetPath}\" -dNCATestInstaller.Msi.TargetPath=\"{outputFileName_Msi}\" -out \"{wixprojFilePath_Exe}\"";
                }
